...
router.get('/config', (req, res) => {
  ...
  res.json({
    ...
    clientId,
    apiKey,
    appId: clientId.split('-')[0], // Project number extracted from client ID
  ...
  try {
    ...
    for (const file of files) {
      try {
        ...
        // Check MIME type — warn but still try for uncommon types
        ...
        const isKnownType = supportedTypes.includes(file.mimeType);
