...
async function processExhibitPackage(jobId, exhibitsData, uploadedFiles, tempDir, config) {
  try {
    ...
    // ─── Drive-imported files (already downloaded to uploads/) ───
    // When a user imports files from Google Drive, the driveImport
    // endpoint saves them to /uploads/ and the frontend passes the
    // server filename in the exhibit metadata. We add those to the
    // uploadedFilesMap so the rest of the pipeline treats them the
    // same as regular uploads.
    for (const exhibit of exhibitsData) {
      if (exhibit.serverFilename && !uploadedFilesMap.has(exhibit.id)) {
        const serverPath = path.join(__dirname, '../../uploads', path.basename(exhibit.serverFilename));
        if (fs.existsSync(serverPath)) {
          const ext = path.extname(exhibit.serverFilename);
          uploadedFilesMap.set(exhibit.id, {
            path: serverPath,
            originalname: exhibit.serverFilename,
            ext: ext
          });
        } else {
          await jobStorage.addLog(jobId, `Drive file not found on server: ${exhibit.serverFilename}`);
        }
      }
    }
